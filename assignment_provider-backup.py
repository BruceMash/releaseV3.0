...
def _debug_task_context(tag: str,
                        state: dict[str, Any] | None = None,
                        event: dict[str, Any] | None = None,
                        pending_task_positions: dict[str, Any] | None = None,
                        provider_context: dict[str, Any] | None = None,
                        task_count: int | None = None) -> None:
    """打印任务可用性相关上下文，帮助排查调用链是否传入了激活状态。"""
    provider_context = provider_context or {}
    task_positions = provider_context.get('task_positions')
    pending_task_ids = provider_context.get('pending_task_ids')
    available_agent_names = provider_context.get('available_agent_names')

    state_tasks = state.get('tasks') if isinstance(state, dict) else None
    task_items = list(state_tasks.items()) if isinstance(state_tasks, dict) else []
    sample_tasks = []
    for task_id, task in task_items[:3]:
        if not isinstance(task, dict):
            sample_tasks.append((str(task_id), type(task).__name__))
            continue
        sample_tasks.append({
            'task_id': str(task_id),
            'keys': sorted(task.keys()),
    ...
    remaining_task_ids = event.get('remaining_task_ids') if isinstance(event, dict) else None

    print(
        f"[AdapterDebug:{tag}] task_count={task_count}, "
        f"state.tasks={len(task_items)}, "
        f"task_positions={len(task_positions) if isinstance(task_positions, dict) else 0}, "
        f"pending_task_ids={len(pending_task_ids) if isinstance(pending_task_ids, list) else 0}, "
        f"pending_task_positions={len(pending_task_positions) if isinstance(pending_task_positions, dict) else 0}, "
        f"available_agents={len(available_agent_names) if isinstance(available_agent_names, list) else 'n/a'}, "
        f"event.type={event_type}, "
        f"event.remaining_task_ids={len(remaining_task_ids) if isinstance(remaining_task_ids, list) else 0}"
    )
    if sample_tasks:
        print(f"[AdapterDebug:{tag}] sample state.tasks: {sample_tasks}")
    elif isinstance(state, dict):
        print(f"[AdapterDebug:{tag}] state has no task metadata")
...
def plan_assignments(
    ...
    print(
        f"[AdapterDebug:plan] parsed scenario -> "
        f"platform_configs={len(platform_configs)}, target_configs={len(target_configs)}, "
        f"n_platforms={n_platforms}, n_targets={n_targets}, phase={phase}"
    )
