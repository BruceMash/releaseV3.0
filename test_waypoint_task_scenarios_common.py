...
def build_collaborative_slot_positions(env, task_center, assigned_agents, args=None, scenario_payload=None):
    ...
    desired_spacing = collab_slot_spacing(env)
...
def attach_collaborative_slot_targets(env, tasks, args=None, scenario_payload=None):
    for task in tasks.values():
        assigned_agents = list(task.get("agents", []))
        if not assigned_agents:
            task.pop("slot_positions", None)
            continue
        task["slot_positions"] = build_collaborative_slot_positions(
            env,
            task["position"],
            assigned_agents,
            args=args,
...
def resolve_agent_queues_from_provider_payload(
    agent_names,
    provider_payload,
    allowed_task_ids=None,
    preserve_collaborative_tasks=False,
):
    if not isinstance(provider_payload, dict):
        return None
    ...
    if preserve_collaborative_tasks:
        return normalized
