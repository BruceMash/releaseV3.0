...
class TaskSystem:
    ...
    def get_action_mask(self, assigned_targets: Set[int], completed_targets: Set[int],
        ...
        # 调试输出
        if current_platform_id < 3 and sum(mask[:-1]) == 0:
    ...
    def assign(self, evaluate: bool = False) -> Tuple[Dict[int, int], List[int], np.ndarray]:
        """执行任务分配 - 修复时序约束检查"""
        ...
        # 顺序分配
        assignment = {}
        assigned_targets = set()  # 多平台协同约束：已分配的目标
        completed_targets = set()  # 时序约束：本轮视为"已完成"的目标（用于检查后续目标）
        actions = []
        ...
        for i in range(config.N_PLATFORMS):
            ...
            # 使用本地completed_targets检查可用性
            action_mask = self.get_action_mask(assigned_targets, completed_targets, i).cpu().numpy()

            if np.sum(action_mask[:-1]) == 0:
                actions.append(action_dim - 1)  # 空闲（最后一个索引）
                continue
            ...
            if action < len(self.env.targets):
                ...
                # 双重检查时序：基于本轮状态（严格防止违反）
                if target.predecessors and not all(pid in completed_targets for pid in target.predecessors):
                    final_action = len(self.env.targets)  # 强制idle
                    temporal_violations += 1
                elif action not in assigned_targets:
                    # 满足3个约束：可分配
                    assignment[i] = action
                    assigned_targets.add(action)
                    completed_targets.add(action)  # 同时更新时序状态
                    self.env.mark_target_completed(action)  # 同步到全局
                else:
                    # 违反多平台协同约束（重复分配）
                    final_action = len(self.env.targets)

            actions.append(final_action)
        unassigned_platforms = [i for i in range(config.N_PLATFORMS) if i not in assignment]

        if unassigned_platforms:
            # 获取剩余可用目标（未被分配且满足时序）
            available_targets = []
            for j, t in enumerate(self.env.targets):
                if j in assigned_targets:
                    continue
                # 检查时序约束
                if t.predecessors and not all(pid in completed_targets for pid in t.predecessors):
                    continue
                available_targets.append((j, t))

            # 按价值排序，优先分配高价值目标
            available_targets.sort(key=lambda x: x[1].value - x[1].threat * 0.3, reverse=True)

            # 为每个未分配平台强制分配（轮询可用目标）
            for pid in unassigned_platforms:
                platform = self.env.platforms[pid]
                assigned = False

                # 优先匹配能力相符的目标
                for tid, target in available_targets:
                    if not platform.can_do(target.task_type):
                        continue
                    ...
                    actions[pid] = tid  # 更新动作记录
                    assigned = True

                    # 从可用列表移除
                    available_targets.remove((tid, target))
                    break

                # 如果能力匹配失败，放宽条件强制分配任意可用目标（确保不Idle）
                if not assigned and available_targets:
                    tid, target = available_targets[0]
                    assignment[pid] = tid
                    assigned_targets.add(tid)
                    completed_targets.add(tid)
                    self.env.mark_target_completed(tid)
                    ...
                    print(f"平台{pid}暂时分配目标{tid}")

        elapsed = time.time() - start_time
        self.stats['times'].append(elapsed)  # 确保记录时间
        return assignment, actions, hu.cpu().numpy()

    def calc_reward(self, assignment: Dict[int, int], completed_snapshot: set = None) -> float:
        if not assignment:
            self.diag['empty_assignments'] += 1
            return -20.0
        n_assigned_platforms = len(assignment)
        coverage_ratio = n_assigned_platforms / config.N_PLATFORMS
        unique_targets = len(set(assignment.values()))
        target_coverage_ratio = unique_targets / config.N_TARGETS

        # 鼓励分配更多目标，而非仅满足平台数
        # 分配10个不同目标：奖励10分
        # 分配16个不同目标（全部分配）：奖励25分
        if unique_targets == config.N_TARGETS:
            target_coverage_bonus = 30.0  # 全部分配大奖
        elif unique_targets >= config.N_TARGETS * 0.8:  # 12个以上
            target_coverage_bonus = 15.0 + (unique_targets - config.N_TARGETS * 0.8) * 2
        else:
            target_coverage_bonus = unique_targets * 1.0  # 基础奖励

        # 惩罚重复目标（鼓励多样性）
        duplicates = len(assignment) - unique_targets
        diversity_penalty = duplicates * 3.0  # 比原来更强的惩罚

        # 原有覆盖奖励调整为基于目标数而非平台数
        coverage_bonus = target_coverage_ratio * 20.0  # 原为coverage_ratio*15

        # 全分配大奖（基于目标数而非平台数）
        full_coverage_bonus = 25.0 if unique_targets == config.N_TARGETS else 0.0

        # 部分分配惩罚（如果目标数<16，即使平台全分配也惩罚）
        partial_penalty = 0.0
        if target_coverage_ratio < 1.0:
            partial_penalty = (1.0 - target_coverage_ratio) * 15.0  # 未分配目标惩罚
        elif coverage_ratio < 1.0:  # 80%-100%之间，轻微惩罚未完成部分
            partial_penalty = (1.0 - coverage_ratio) * 3.0

        # 1. 计算基础目标函数（航程+时间+攻击收益）
        total_dist = 0.0
        max_time = 0.0
        ...
        for pid, tid in assignment.items():
            if pid >= len(self.env.platforms) or tid >= len(self.env.targets):
                continue
            ...
            if cache_key not in self._distance_cache:
                dist = self.route_est.estimate_distance(p.position, t.position)
                self._distance_cache[cache_key] = dist
            else:
                dist = self._distance_cache[cache_key]

            normalized_dist = dist / (map_diag + 1e-6)
            total_dist += normalized_dist

            time_cost = dist / max(p.speed, 1e-6)
            max_time = max(max_time, time_cost / 10000.0)
        ...
        obj = max(obj, 0.01)
        base_reward = 20.0 / (1.0 + abs(obj) + 1e-6)
        ...
        if duplicates > 0:
            ...
            duplicate_penalty = 2.0 * duplicates  # 每个重复惩罚2.0
            base_reward *= (0.7 ** duplicates)  # 指数衰减
        else:
            duplicate_penalty = 0.0
            base_reward *= 1.15  # 无重复奖励加成
        ...
        for tid in assignment.values():
            ...
            if target.predecessors:
                ...
                if all(pid in check_set for pid in target.predecessors):
                    temporal_bonus += 0.8  # 满足时序约束奖励
                else:
                    temporal_violations += 1
                    temporal_bonus -= 2.0  # 时序违反惩罚

        # 平台类型约束：已通过mask保证满足，给予固定奖励
        capability_bonus = n_assigned_platforms * 1.5  # 每个分配的平台给予奖励
        ...
        final_reward = (base_reward +
                        coverage_bonus +
                        full_coverage_bonus +
                        target_coverage_bonus +  # 新增：目标覆盖奖励
                        temporal_bonus +
                        capability_bonus -
                        duplicate_penalty -
                        diversity_penalty -  # 新增：多样性惩罚
                        partial_penalty)
        if unique_targets == config.N_TARGETS:
            # 全分配时，无论其他因素如何，奖励至少15分（显著高于部分分配的5-10分）
            final_reward = max(final_reward, 15.0)
        elif unique_targets >= 14:
            # 14-15目标保底10分
            final_reward = max(final_reward, 10.0)
        elif unique_targets <= 10 and n_assigned_platforms == config.N_PLATFORMS:
            # 惩罚"10平台各1目标"的次优解：虽然平台全分配，但目标太少
            final_reward -= 5.0  # 额外惩罚，打破当前僵局
        # 5. 归一化与裁剪
        ...
        if len(self.reward_history) >= 100:
            ...
            normalized_reward = np.clip(normalized_reward, -8, 8)  # 放宽裁剪范围
        else:
            normalized_reward = final_reward

        if n_assigned_platforms > 0 and normalized_reward < 0.05:
            normalized_reward = 0.05 + (n_assigned_platforms / config.N_PLATFORMS) * 0.1
        if n_assigned_platforms == config.N_PLATFORMS and normalized_reward < 5.0:
            normalized_reward = 5.0  # 强制提升全分配奖励到显著高于部分分配
    ...
    def train_step(self):
        ...
        # 定期打印诊断信息（每1000步）
        if len(self.masac.buffer) % 1000 == 0 and len(self.masac.buffer) > 0:
            print(f"\n=== 训练诊断 (Buffer: {len(self.masac.buffer)}) ===")
            print(
                f"空分配: {self.diag['empty_assignments']}, 重复: {self.diag['duplicate_hits']}, 有效奖励: {self.diag['valid_rewards']}")
            if len(self.reward_history) > 0:
                print(f"奖励范围: {min(self.reward_history):.3f} ~ {max(self.reward_history):.3f}")
                print(f"最近10个奖励: {[f'{r:.2f}' for r in list(self.reward_history)[-10:]]}")
...
class TerrainTrainer(Trainer):
    ...
    def train(self, episodes=10000):
        ...
        for episode in range(episodes):
            ...
            # 时序违反统计
            temporal_viol = 0
            for pid, tid in temp_assignment.items():
                target = self.env.targets[tid]
                if target.predecessors:
    ...
    def visualize_3d(self, assignment: Dict[int, int]):
        """3D可视化 - 修复：合并两个方法，移除super()调用"""
        ...
        for p in self.env.platforms:
            ...
            ax.text(p.position[0], p.position[1], p.position[2] + 8000,
                    f'P{p.id}', fontsize=11, fontweight='bold', zorder=15,
        ...
        # 绘制分配连线
        for pid, tid in assignment.items():
            if pid < len(self.env.platforms) and tid < len(self.env.targets):
                p_pos = self.env.platforms[pid].position
                t_pos = self.env.targets[tid].position

                ax.plot([p_pos[0], t_pos[0]], [p_pos[1], t_pos[1]], [p_pos[2], t_pos[2]],
                        'k-', linewidth=3, alpha=0.6, zorder=5)

                mid = (p_pos + t_pos) / 2
                direction = (t_pos - p_pos)
        ...
        title = (f'智能空面协同任务分配三维可视化 (修复版v4.3)\\n'
                 f'{len(assignment)}个唯一分配 | {len(self.env.threats)}个威胁区域 | '
                 f'分配时间: {avg_time:.2f}ms | 约束满足率: 100%')
    ...
    def _plot_temporal_dependencies_simple(self, assignment: Dict[int, int], episode: int):
        ...
        assigned_targets = list(assignment.values())
        platform_to_target = {pid: tid for pid, tid in assignment.items()}
        ...
        for platform_id in range(config.N_PLATFORMS):
            ...
            for idx, assignment in enumerate(recent_assignments):
                ...
                if platform_id in assignment:
                    target_id = assignment[platform_id]
