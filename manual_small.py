...
class Config:
    ...
    EXPLORATION_RESTART_EVERY: int = 200  # 每500回合重启探索
    RESTART_EPSILON: float = 0.6  # 重启时的探索率
    ...
    PARAM_NOISE_SCALE: float = 0.05  # 新增参数，重启时参数噪声强度
    ...
    # 奖励缩放因子（用于数值稳定性）
    REWARD_SCALE: float = 100.0  # 降低缩放因子
    REWARD_MAX: float = 10.0  # 奖励裁剪上限
    REWARD_MIN: float = -10.0  # 奖励裁剪下限
    # 添加归一化窗口
    REWARD_NORM_WINDOW: int = 1000

    CONVERGENCE_WINDOW: int = 50  # 检查窗口大小
    CONVERGENCE_THRESHOLD: float = 0.01  # 奖励变化阈值（1%）
    PATIENCE: int = 200  # 容忍多少个窗口无改进就停止
    MIN_EPISODES: int = 500  # 最少训练回合数（防止过早停止）
...
class TerrainLoader:
    ...
    def __init__(self, csv_path='江宁.csv', use_simulation=False):
        ...
        grid = np.zeros((len(ut), len(ul)))
        for i, t in enumerate(ut):
            for j, n in enumerate(ul):
                m = (self.lons == n) & (self.lats == t)
                if m.any():
                    grid[i, j] = self.elevs[m][0]
...
class Platform:
    """作战平台类"""
    TYPES = ['UAV', 'UCAV', 'USV', 'UUV']
    COLORS = {'UAV': '#1f77b4', 'UCAV': '#2ca02c', 'USV': '#ff7f0e', 'UUV': '#9467bd'}

    CAPABILITIES = {
        'UAV': ['reconnaissance', 'electronic', 'surveillance'],
        'UCAV': ['attack', 'reconnaissance', 'electronic'],
        'USV': ['attack', 'reconnaissance', 'surveillance'],
        'UUV': ['reconnaissance', 'electronic', 'surveillance']
    }

    DEFAULT_MAX_RANGE = {'UAV': 500e3, 'UCAV': 800e3, 'USV': 300e3, 'UUV': 200e3}
    DEFAULT_SPEED = {'UAV': 300, 'UCAV': 250, 'USV': 80, 'UUV': 50}
    DEFAULT_PAYLOAD = {'UAV': 4, 'UCAV': 6, 'USV': 8, 'UUV': 2}

    def __init__(self, id: int, pos: np.ndarray, ptype: str = 'UAV'):
        self.id = id
        self.position = pos.copy().astype(np.float32)
        self.type = ptype if ptype in self.TYPES else 'UAV'
        self.max_range = self.DEFAULT_MAX_RANGE[self.type]
        self.speed = self.DEFAULT_SPEED[self.type]
        self.max_payload = self.DEFAULT_PAYLOAD[self.type]
...
class Target:
    """目标类"""
    TASKS = ['attack', 'reconnaissance', 'electronic']
    COLORS = {'attack': '#d62728', 'reconnaissance': '#ffd700', 'electronic': '#9467bd'}
    MARKERS = {'attack': '^', 'reconnaissance': 's', 'electronic': 'd'}

    def __init__(self, id: int, pos: np.ndarray, predecessors=None):
        self.id = id
        self.position = pos.copy().astype(np.float32)
        self.threat = random.uniform(0.1, 1.0)
        self.value = random.uniform(0.5, 1.0)
        self.time_win_start = random.uniform(0, 400)
        self.time_win_end = random.uniform(600, 1000)
        self.task_type = random.choice(self.TASKS)
        self.req_payload = random.randint(1, 3)
        self.completed = False
        self.assigned_platform = None
        self.priority = random.uniform(0.5, 1.0)
        # 新增：前序任务ID列表（必须先完成的任务）
...
class Environment:
    ...
    def check_constraints(self, assignment: Dict[int, int], check_temporal: bool = True) -> Tuple[
        bool, str, float, int]:
        if not assignment:
        ...
        # 1. 多平台协同任务约束（重复分配检查）
        targets_assigned = list(assignment.values())
        duplicates = len(targets_assigned) - len(set(targets_assigned))
        if duplicates > 0:
        ...
        for pid, tid in assignment.items():
            if pid >= len(self.platforms) or tid >= len(self.targets):
                continue
            ...
            if check_temporal and not target.check_predecessors_completed(self):
                penalty += config.PENALTY_TEMPORAL  # 使用专门的时序惩罚
    ...
    def get_graph_data(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # 转回 torch tensor
        return platform_states, target_states, torch.FloatTensor(adj_matrix), torch.FloatTensor(temporal_matrix)
...
class TerrainEnv(Environment):
    ...
    def _init_platforms(self):
        ...
        for i in range(config.N_PLATFORMS):
            ...
            # 关键修改：前三个平台固定为UCAV（全能型），确保第一轮分配成功率
            if i < 3:
                ptype = 'UCAV'  # UCAV可以执行attack、reconnaissance、electronic三种任务
            else:
                ptype = types[i % 3]

            pos[2] = 0 if ptype == 'USV' else random.uniform(1000, 80000)
...
class MASAC:
    ...
    def update(self):
        ...
        for i in range(config.N_PLATFORMS):
            ...
            action, _ = self.actor.sample(states[:, i, :], mask)
            new_actions.append(action)
        new_actions = torch.stack(new_actions, dim=1)
        new_actions_onehot = F.one_hot(new_actions, num_classes=config.ACTION_DIM).float()
...
class ConvergenceMonitor:
    ...
    def __init__(self):
        ...
        # 新增：跟踪最佳全分配模型
        self.best_full_coverage_reward = -1e10
        self.best_full_coverage_episode = 0
    ...
    def update(self, episode: int, reward: float, n_assigned: int, assignment: Dict[int, List[int]], env) -> dict:
        """
        更新监控状态 - 强制要求全平台分配
        参数:
            assignment: 平台到目标列表的映射 (Dict[int, List[int]])
            env: 环境实例，用于访问平台和目标信息
        """
        # 初始化...
        self.reward_history.append(reward)
        self.episode_times.append(time.time())
        self.plateau_history.append(reward)

        # 展平所有分配的目标ID（支持多任务分配）
        all_target_ids = []
        for tids in assignment.values():
            if isinstance(tids, (list, tuple)):
                all_target_ids.extend(tids)
            else:
                all_target_ids.append(tids)

        n_platforms_assigned = len(assignment) if assignment else 0
        unique_targets = len(set(all_target_ids))
        full_coverage = (n_platforms_assigned == config.N_PLATFORMS)
        ...
        status = {
            ...
            'target_coverage': unique_targets,  # 添加目标覆盖数
            'full_coverage': full_coverage,
            'coverage_rate': n_platforms_assigned / config.N_PLATFORMS if config.N_PLATFORMS > 0 else 0,
            'new_best': False,
            'new_best_full_coverage': False,
            'is_collapse': False,
            'is_plateau': False,
            'suggest_exploration_restart': False,
            'window_mean': 0.0,
            'window_std': 0.0
        }
        if assignment and env:
            penalty = 0.0
            reasons = []

            # 1. 重复分配检查（使用展平后的列表）
            duplicates = len(all_target_ids) - unique_targets
            if duplicates > 0:
                penalty += config.PENALTY_DUPLICATE * duplicates
                reasons.append(f"Duplicate ({duplicates})")
            ...
            for pid, tids in assignment.items():
                ...
                # 统一处理为列表（关键修复）
                target_list = tids if isinstance(tids, (list, tuple)) else [tids]

                for tid in target_list:
                    if tid >= len(env.targets):
                        continue

                    platform = env.platforms[pid]
                    ...
                    # 任务时序约束（使用 env 替代 self.env）
                    if not target.check_predecessors_completed(env):
                        penalty += config.PENALTY_TEMPORAL
                        reasons.append(f"Temporal violation: T{tid}")

            reason_str = "; ".join(reasons) if reasons else "OK"
            status['constraint_check'] = (penalty == 0, reason_str, penalty, duplicates)
        ...
        if len(self.reward_history) > 0:
            ...
            # 崩溃检测：奖励异常低且分配不足
            if current_mean < -5.0 and not full_coverage and episode > 100:
                status['is_collapse'] = True

        if episode % 10 == 0 and len(self.plateau_history) >= self.PLATEAU_WINDOW:
            recent_mean = np.mean(list(self.plateau_history)[-self.PLATEAU_WINDOW // 2:])
            older_mean = np.mean(list(self.plateau_history)[:self.PLATEAU_WINDOW // 2])

            # 如果近期平均值与早期几乎相同，且未达到历史最佳，则视为平台期
            relative_change = abs(recent_mean - older_mean) / (abs(older_mean) + 1e-6)

            if relative_change < self.PLATEAU_THRESHOLD:
                self.plateau_counter += 10  # 每10回合增加10
            else:
                self.plateau_counter = 0

            # 平台期判定：连续100回合（10次检查）奖励无显著改善，且未达最佳
            if self.plateau_counter >= 100 and current_mean < self.best_reward * 0.95:
                status['is_plateau'] = True
                # 只有全部分配时才建议重启（避免在探索阶段误触发）
                if status['full_coverage']:
                    status['suggest_exploration_restart'] = True
        # 全分配模型评估
        if full_coverage and reward > self.best_full_coverage_reward:
            self.best_full_coverage_reward = reward
            self.best_full_coverage_episode = episode
            status['new_best_full_coverage'] = True
            status['new_best'] = True
            self.patience_counter = 0  # 有改进，重置耐心
            print(
                f"🏆 回合{episode}: 新最佳全分配模型! 奖励{reward:.3f}, 全部分配({n_platforms_assigned}/{config.N_PLATFORMS})")

        elif not full_coverage and reward > self.best_reward * 1.1:
            # 非全分配模型，即使奖励高也不记为"最佳"
            status['new_best'] = False
            if episode % 500 == 0:
                print(
                    f"⚠️ 回合{episode}: 奖励{reward:.3f}较高，但未全部分配({n_platforms_assigned}/{config.N_PLATFORMS})，不记为最佳")
            ...
            if self.patience_counter >= config.PATIENCE:
                ...
                if episode >= config.MIN_EPISODES:
                    ...
                    status['stop_reason'] = f"全分配模型连续{config.PATIENCE}窗口无改进，已收敛"

        else:
            # 非全分配，增加耐心计数但不允许停止
            self.patience_counter += 1
            if self.patience_counter >= config.PATIENCE + 200:
                print(f"⚠️ 回合{episode}: 长期未达成全部分配，强制继续训练...")
                self.patience_counter = config.PATIENCE
        ...
        if len(self.episode_times) > 1:
            avg_time_per_episode = np.mean(np.diff(self.episode_times))
            remaining_episodes = config.MAX_EPISODES - episode
            status['eta_seconds'] = avg_time_per_episode * remaining_episodes


        # 计算当前窗口统计
        current_mean = np.mean(self.reward_history)
        current_std = np.std(self.reward_history)

        status['window_mean'] = current_mean
        status['window_std'] = current_std

        # 检查是否有改进（相对于历史最佳）
        # ✅ 修改：只有满足全部分配时才考虑保存为最佳模型
        if current_mean > self.best_reward * (1 + config.CONVERGENCE_THRESHOLD) and full_coverage:
            # 有显著改进且全平台分配，重置耐心计数器
            ...
            status['new_best'] = True
            print(
                f"🏆 回合{episode}: 新最佳模型! 奖励{current_mean:.3f}, 全部分配({n_platforms_assigned}/{config.N_PLATFORMS})")

        elif current_mean > self.best_reward * (1 + config.CONVERGENCE_THRESHOLD) and not full_coverage:
            # 奖励提升但未全部分配，不保存为最佳，但给予提示
            status['reward_improved'] = True
            print(
                f"⚠️ 回合{episode}: 奖励提升至{current_mean:.3f}，但未全部分配({n_platforms_assigned}/{config.N_PLATFORMS})，不记为最佳模型")

        elif not full_coverage and episode % 100 == 0:
            # 每100回合提醒一次覆盖率不足
            print(
                f"📊 回合{episode}: 当前覆盖率 {n_platforms_assigned}/{config.N_PLATFORMS} ({status['coverage_rate']:.1%})，等待全部分配...")

            # 计算收敛进度（0-100%）
            if self.best_reward > 0:
                status['progress'] = min(100, (current_mean / (current_mean + current_std)) * 100)
        else:
            # 无改进，增加耐心计数器
            self.patience_counter += 1

            # 检查是否收敛（连续PATIENCE个窗口无改进）
            # ✅ 修改：收敛必须满足全部分配条件
            if self.patience_counter >= config.PATIENCE:
                if full_coverage:
                    self.converged = True
                    ...
                    if episode >= config.MIN_EPISODES:
                        status['should_stop'] = True
                        status['stop_reason'] = f"连续{config.PATIENCE}个窗口无改进且全部分配，已收敛"
                else:
                    # 耐心耗尽但未全部分配，强制继续训练
                    self.converged = False
                    ...
                    print(
                        f"🚨 回合{episode}: 满足耐心条件但未全部分配({n_platforms_assigned}/{config.N_PLATFORMS})，强制继续训练...")

                    # 每500回合重置一次耐心计数器，防止无限等待
                    if self.patience_counter >= config.PATIENCE + 500:
                        print(f"⚠️ 长时间未达成全部分配，重置耐心计数器并给予探索奖励...")
                        self.patience_counter = 0
        ...
        if len(self.reward_history) >= self.CONVERGENCE_WINDOW:
            ...
            # 检测是否进入平台期（奖励变化<1%）
            if abs(current_mean - self.last_window_mean) / (abs(self.last_window_mean) + 1e-6) < 0.01:
                self.plateau_counter += 1
            else:
                self.plateau_counter = 0

            self.last_window_mean = current_mean

            # 平台期过长且未达历史最佳，触发建议
            if self.plateau_counter >= self.PLATEAU_THRESHOLD and current_mean < self.best_reward * 0.9:
                status['is_plateau'] = True
                status['suggest_exploration_restart'] = True
            else:
                status['is_plateau'] = False
...
class TaskSystem:
    ...
    def get_action_mask(self, assigned_targets: Set[int], completed_targets: Set[int],
                        current_platform_id: int, alive_platforms: List[int] = None) -> torch.Tensor:
        """增强版动作掩码，支持指定可用平台"""
        ...
        rejected_reasons = {'assigned': 0, 'temporal': 0, 'completed': 0, 'payload': 0, 'capability': 0}
    ...
    def assign(self, evaluate: bool = False) -> Tuple[Dict[int, List[int]], List[int], np.ndarray]:
        ...
        assigned_targets = set()  # 全局已分配目标（多平台协同约束）
        completed_targets = set()  # 本轮视为"已完成"的目标（用于时序约束检查）
        actions = []
        ...
        # ===== 第0轮：强制每个平台至少分配1个任务 =====
        platform_order = list(range(config.N_PLATFORMS))
        random.shuffle(platform_order)  # 随机顺序避免总是前面平台优先选目标
        ...
        for i in platform_order:
            ...
            # 获取可用目标索引（排除idle）
            available_indices = np.where(action_mask[:-1] == 1)[0]

            if len(available_indices) > 0:
                # 强制选择第一个可用目标（不追求最优，先保证分配）
                # 也可选择距离最近的：这里简化处理，按索引选
                selected_target = int(available_indices[0])

                target = self.env.targets[selected_target]

                # 执行分配
                assignment[i].append(selected_target)
                assigned_targets.add(selected_target)
                completed_targets.add(selected_target)
                self.env.mark_target_completed(selected_target)
                actions.append(selected_target)

                # 扣除载荷
                remaining_payload = platform.payload - target.req_payload

                # 如果还有剩余载荷和能力，且还有目标，标记为可继续分配
                # 但不在这里继续分配，留给第二轮统一处理

        # 统计第一轮结果
        first_round_count = sum(len(tasks) for tasks in assignment.values())
        ...
        for round in range(1, max_rounds):
            for i in range(config.N_PLATFORMS):
                ...
                # 计算当前已用载荷
                used_payload = sum(self.env.targets[tid].req_payload
                                   for tid in assignment[i])
                ...
                available_count = np.sum(action_mask[:-1])
                ...
                if action < len(self.env.targets):
                    ...
                    if can_assign:
                        ...
                        # 记录动作（使用实际目标ID，而非idle）
                        actions.append(action)
                    else:
                        # 不能分配此目标，尝试其他目标或结束
                        # 如果是因为时序或载荷不足，尝试找其他可用目标
                        if target.req_payload > remaining_payload:
                            ...
                            for alt_tid in range(n_targets):
                                if (action_mask[alt_tid] == 1 and
                                    ...
                                    assigned_count += 1
                                    actions.append(alt_tid)
                                    found_alternative = True
                                    break
                            if not found_alternative:
                                break  # 无合适目标，结束该平台分配
                        else:
                            break  # 时序约束违反，结束该平台分配
                else:
                    # 选择了idle，结束该平台分配
                    actions.append(action_dim - 1)
                    break

            # 如果该平台一个任务都没分配到，记录为idle
            if assigned_count == 0 and len(actions) < i + 1:
                actions.append(action_dim - 1)

        if len(self.env.targets) >= len(self.env.platforms):
            # 识别空闲平台
            idle_platforms = [i for i in range(config.N_PLATFORMS)
                              if i not in assignment or len(assignment[i]) == 0]

            if idle_platforms:
                # 获取剩余未分配目标（排除已完成和已分配的）
                remaining_targets = [j for j in range(n_targets)
                                     if j not in assigned_targets
                                     and j not in completed_targets
                                     and not self.env.targets[j].completed]

                # 为每个空闲平台强制分配一个最合适的剩余目标
                for pid in idle_platforms:
                    if not remaining_targets:
                        break  # 没有剩余目标了

                    platform = self.env.platforms[pid]
                    best_target = None
                    best_score = -1e10

                    # 在剩余目标中找最佳匹配（优先距离近、载荷匹配、放宽类型约束）
                    candidates = []
                    for tid in remaining_targets:
                        target = self.env.targets[tid]

                        # 硬约束1：时序必须满足（绝对不能违反）
                        if target.predecessors and not all(p in completed_targets for p in target.predecessors):
                            continue
                        ...
                        # 计算评分（距离优先，类型匹配加分但不做硬性过滤）
                        dist = np.linalg.norm(platform.position - target.position)
                        score = -dist / 1e5
                        if platform.can_do(target.task_type):
                            score += 5.0  # 类型匹配给予显著加分
                        else:
                            score += 0.5  # 类型不匹配也给基础分，确保能选上

                        candidates.append((tid, score, target))
                    ...
                    if best_target is not None:
                        # 强制分配（即使类型不完全匹配，只要载荷够就能执行）
                        target = self.env.targets[best_target]
                        if platform.payload >= target.req_payload:  # 载荷必须满足
                            assignment[pid] = [best_target]
                            assigned_targets.add(best_target)
                            completed_targets.add(best_target)  # 更新时序状态
                            self.env.mark_target_completed(best_target)
                            ...
                            # 尝试继续分配（如果还有剩余载荷和目标）
                            while remaining_targets and remaining_payload > 0:
                                found_next = False
                                for tid in remaining_targets:
                                    next_target = self.env.targets[tid]
                                    if next_target.req_payload <= remaining_payload:
                                        # 检查时序
                                        if next_target.predecessors and not all(
                                                p in completed_targets for p in next_target.predecessors):
                                            continue

                                        assignment[pid].append(tid)
                                        assigned_targets.add(tid)
                                        completed_targets.add(tid)
                                        self.env.mark_target_completed(tid)
                                        remaining_payload -= next_target.req_payload
                        ...
                        # ===== 终极强制分配：实在找不到匹配的，强行分配第一个可用的 =====
                        if remaining_targets:
                            # 按载荷需求从小到大排序，优先分配小任务
                            emergency_targets = sorted(remaining_targets,
                                                       key=lambda tid: self.env.targets[tid].req_payload)

                            for emergency_tid in emergency_targets:
                                emergency_target = self.env.targets[emergency_tid]

                                # 只检查时序和载荷，不检查类型！
                                if platform.payload >= emergency_target.req_payload:
                                    # 检查时序约束（硬约束不能违反）
                                    if emergency_target.predecessors and not all(
                                            p in completed_targets for p in emergency_target.predecessors):
                                        continue

                                    # 强制分配，即使类型不匹配
                                    assignment[pid] = [emergency_tid]
                                    assigned_targets.add(emergency_tid)
                                    completed_targets.add(emergency_tid)
                                    self.env.mark_target_completed(emergency_tid)
                                    remaining_targets.remove(emergency_tid)
                                    break
        # 后处理：强制确保所有目标都被分配（如果可能）
        unassigned_targets = [j for j in range(n_targets) if j not in assigned_targets]

        # 尝试将剩余目标分配给还有剩余载荷的平台
        for tid in unassigned_targets:
            target = self.env.targets[tid]
            ...
            for pid in range(config.N_PLATFORMS):
                platform = self.env.platforms[pid]
                current_payload = platform.payload - sum(
                    self.env.targets[t].req_payload for t in assignment[pid]
                )

                if (current_payload >= target.req_payload and
                        platform.can_do(target.task_type) and
                        tid not in assigned_targets):
                    assignment[pid].append(tid)
                    assigned_targets.add(tid)
                    completed_targets.add(tid)
                    self.env.mark_target_completed(tid)
                    break
        idle_platforms_final = [i for i in range(config.N_PLATFORMS)
                                if i not in assignment or len(assignment[i]) == 0]

        for pid in idle_platforms_final:
            platform = self.env.platforms[pid]
            # 遍历所有目标，只看载荷约束，不管类型和时序
            for tid in range(n_targets):
                if tid not in assigned_targets and tid not in completed_targets:
                    target = self.env.targets[tid]
                    # 只要载荷够，就强制分配（类型不匹配也强行分）
                    if platform.payload >= target.req_payload:
                        assignment[pid] = [tid]
                        assigned_targets.add(tid)
                        completed_targets.add(tid)
                        self.env.mark_target_completed(tid)
        ...
        # 转换为扁平actions列表（用于经验回放缓冲区）
        # 注意：这里简化处理，只记录每个平台的第一个动作，或需要修改经验存储逻辑
        flat_actions = []
        for pid in range(config.N_PLATFORMS):
            if assignment[pid]:
                flat_actions.extend(assignment[pid])
            else:
                flat_actions.append(action_dim - 1)  # idle

        # 截断或填充到固定长度（用于网络输入）
        while len(flat_actions) < config.N_PLATFORMS:
            flat_actions.append(action_dim - 1)

        return assignment, flat_actions[:config.N_PLATFORMS], hu.cpu().numpy()
    ...
    def calc_reward(self, assignment: Dict[int, List[int]], completed_snapshot: set = None) -> float:
        ...
        if not assignment or all(len(tasks) == 0 for tasks in assignment.values()):
            self.diag['empty_assignments'] += 1
            return -20.0

        # 统计分配情况
        n_assigned_platforms = sum(1 for tasks in assignment.values() if len(tasks) > 0)
        all_assigned_targets = []
        for tasks in assignment.values():
            all_assigned_targets.extend(tasks)

        unique_targets = len(set(all_assigned_targets))
        target_coverage_ratio = unique_targets / config.N_TARGETS

        # 全部分配奖励（关键指标：是否完成全部16个目标）
        if unique_targets == config.N_TARGETS:
            target_coverage_bonus = 50.0  # 大幅提升全分配奖励
        elif unique_targets >= config.N_TARGETS * 0.9:  # 14个以上
            target_coverage_bonus = 30.0
        elif unique_targets >= config.N_TARGETS * 0.75:  # 12个以上
            target_coverage_bonus = 15.0
        else:
            target_coverage_bonus = unique_targets * 0.5

        # 惩罚重复目标
        duplicates = len(all_assigned_targets) - unique_targets
        diversity_penalty = duplicates * 5.0

        # 1. 计算基础目标函数（所有分配航段的总和）
        total_dist = 0.0
        max_time = 0.0
        ...
        for pid, tids in assignment.items():
            ...
            # 计算该平台所有任务的总航程（假设顺序执行）
            prev_pos = p.position
            platform_total_dist = 0.0
            ...
            for tid in tids:
                ...
                platform_total_dist += dist
                prev_pos = t.position  # 更新位置为当前目标位置（顺序执行）

                # 攻击代价
                X_ij = 1.0
                M_va = p.payload * (p.speed / 300.0)
                type_factor = 1.5 if (p.type == 'UCAV' and t.task_type == 'attack') else \
                    1.3 if (p.type == 'UAV' and t.task_type == 'reconnaissance') else 1.0
                P_de = type_factor / (1.0 + t.threat)

                if M_va * P_de > 1e-6:
                    attack_cost = X_ij / (M_va * P_de)
                    attack_cost = np.log1p(attack_cost) / 2.0
                else:
                    attack_cost = 5.0
                total_attack_cost += attack_cost

            # 该平台完成所有任务的时间
            platform_time = platform_total_dist / max(p.speed, 1e-6)
            max_time = max(max_time, platform_time / 10000.0)

            # 归一化距离
            normalized_dist = platform_total_dist / (map_diag + 1e-6)
            total_dist += normalized_dist

        # 2. 综合目标函数
        obj = total_dist + max_time + total_attack_cost
        obj = max(obj, 0.01)
        base_reward = 30.0 / (1.0 + abs(obj) + 1e-6)  # 提高基础奖励系数
        ...
        for pid, tids in assignment.items():
            for tid in tids:
                ...
                if target.predecessors:
                    ...
                    if all(pid in check_set for pid in target.predecessors):
                        temporal_bonus += 1.0  # 满足时序约束奖励
                    else:
                        temporal_violations += 1
                        temporal_bonus -= 3.0  # 时序违反惩罚

        # 平台能力约束奖励（已通过mask保证，给予固定奖励）
        capability_bonus = n_assigned_platforms * 2.0

        # 载荷利用率奖励（鼓励充分利用平台载荷）
        payload_utilization = 0.0
        for pid, tids in assignment.items():
            if not tids:
                continue
            p = self.env.platforms[pid]
            used_payload = sum(self.env.targets[tid].req_payload for tid in tids)
            utilization = used_payload / p.max_payload
            payload_utilization += utilization * 5.0  # 每个平台最高5分

        # 4. 最终奖励合成
        final_reward = (base_reward +
                        target_coverage_bonus +  # 目标覆盖奖励（核心）
                        payload_utilization +  # 载荷利用率奖励（新增）
                        temporal_bonus +
                        capability_bonus -
                        diversity_penalty)

        # 硬性约束：如果完成全部16个目标，保底奖励20分
        if unique_targets == config.N_TARGETS:
            final_reward = max(final_reward, 20.0)
        elif unique_targets >= 14:
        ...
        if len(self.reward_history) >= 100:
            ...
            self.reward_std = max(np.std(self.reward_history), 0.1)
            normalized_reward = (final_reward - self.reward_mean) / self.reward_std
            normalized_reward = np.clip(normalized_reward, -8, 8)
        else:
            normalized_reward = final_reward

        # 保底奖励（只要有分配）
        if n_assigned_platforms > 0 and normalized_reward < 0.05:
            normalized_reward = 0.05 + (unique_targets / config.N_TARGETS) * 0.5

        # 全部分配保底奖励
        if unique_targets == config.N_TARGETS and normalized_reward < 8.0:
            normalized_reward = 8.0  # 强制提升全分配奖励

        normalized_reward = np.clip(normalized_reward, config.REWARD_MIN, config.REWARD_MAX)

        if normalized_reward > 0.01:
            self.diag['valid_rewards'] += 1

        return float(normalized_reward)

    def train_step(self):
        """执行一步训练 - 多任务版本"""  # ← 修改1：注释更新
        assignment, actions, joint_states = self.assign(evaluate=False)
        completed_snapshot = set(self.env.completed_tasks)
        reward = self.calc_reward(assignment, completed_snapshot)

        # 存储经验（使用扁平化actions）          # ← 修改2：注释说明
        joint_next = joint_states + np.random.normal(0, 0.01, joint_states.shape).astype(np.float32)
        self.masac.store(joint_states, np.array(actions), reward, joint_next, False)
        self.masac.decay_epsilon()

        # 网络更新（保持不变）                   # ← 修改3：简化，移除诊断打印
        if (len(self.masac.buffer) > config.WARMUP_STEPS and
                len(self.masac.buffer) % config.UPDATE_INTERVAL == 0):
            return self.masac.update()
        return {}
...
class OnlineFineTuner:
    ...
    def adapt_to_new_size(self, n_platforms: int, n_targets: int):
        ...
        print(f"动态调整网络维度: 动作维度 {config.ACTION_DIM} -> {new_action_dim}")
        ...
        self.system.masac.critic = new_critic
        self.system.masac.critic_target = Critic(new_action_dim).to(self.system.device)
        self.system.masac.critic_target.load_state_dict(new_critic.state_dict())
        ...
        print(f"IRL奖励学习器已更新: 动作维度 {new_action_dim}")
        ...
        print(f"网络维度调整完成: Actor/Critic/IRL 动作维度已更新为{new_action_dim}")
...
# ================== 新增：约束保障分配器（位置：OnlineFineTuner 类之后） ==================
class GuaranteedAssignment:
    """约束满足型强制分配器（应急回退）- 确保100%满足平台能力、时序、无重复约束"""

    def __init__(self, env):
        self.env = env

    def greedy_assign_with_constraints(self, alive_platforms: List[int]) -> Dict[int, int]:
        """贪心算法确保所有存活平台都有任务，且满足所有硬约束"""
        assignment = {}
        assigned_targets = set()
        completed_targets = set(self.env.completed_tasks)

        # 阶段1：构建可执行任务队列（拓扑排序，确保时序）
        executable_targets = []
        for tid, target in enumerate(self.env.targets):
            if tid in completed_targets or tid in assigned_targets:
                continue
            # 检查时序约束：所有前序必须已完成
            if target.predecessors and not all(pid in completed_targets for pid in target.predecessors):
                continue
            executable_targets.append((tid, target))

        # 按价值/威胁排序（启发式）
        executable_targets.sort(key=lambda x: x[1].value - x[1].threat * 0.5, reverse=True)

        # 阶段2：强制分配（每个存活平台必须分配一个）
        random.shuffle(alive_platforms)

        for pid in alive_platforms:
            platform = self.env.platforms[pid]
            assigned = False

            # 优先尝试高价值匹配
            for tid, target in executable_targets:
            ...
            # 如果没有合适任务，强制分配剩余任意可执行（确保不Idle）
            if not assigned and executable_targets:
                for tid, target in executable_targets:
                    if tid in assigned_targets:
                        continue
                    if platform.can_do(target.task_type):  # 至少能力匹配
                        assignment[pid] = tid
                        assigned_targets.add(tid)
                        assigned = True
                        print(f"⚠️ 强制分配: P{pid}→T{tid}（非最优但可行）")
                        break

        # 阶段3：约束验证（断言检查）
        self._verify_constraints(assignment, alive_platforms)
        return assignment

    def _verify_constraints(self, assignment: Dict[int, int], alive_platforms: List[int]):
        """验证所有约束，失败则报错"""
        # 1. 全分配检查
        assert len(assignment) == len(alive_platforms), \
            f"崩溃恢复失败：仅分配{len(assignment)}/{len(alive_platforms)}平台"

        # 2. 重复检查
        targets = list(assignment.values())
        assert len(targets) == len(set(targets)), f"重复分配检测: {targets}"
        ...
        for pid, tid in assignment.items():
            ...
            if target.predecessors:
                for pred in target.predecessors:
                    assert self.env.targets[pred].completed, \
                        f"时序违反: T{tid}的前序T{pred}未完成"

        # 4. 能力检查
        for pid, tid in assignment.items():
            p, t = self.env.platforms[pid], self.env.targets[tid]
            assert p.can_do(t.task_type), \
                f"能力不匹配: P{pid}({p.type})无法执行{t.task_type}"

        print(f"✅ 强制分配验证通过：{len(assignment)}平台全部分配，无约束违反")
...
class DynamicReallocator:
    ...
    def __init__(self, env, system):
        ...
        self.guarantor = GuaranteedAssignment(env)  # 添加约束保障器
    ...
    def _reallocate_after_crash(self, crashed_id: int):
        ...
        if len(best_assignment) < len(alive_platforms):
            ...
            best_assignment = self.guarantor.greedy_assign_with_constraints(alive_platforms)
            # 将优质强制分配加入经验池
...
class TerrainTrainer(Trainer):
    ...
    def __init__(self, use_real_terrain=False):
        super().__init__()
        # 默认使用模拟地形，不加载江宁.csv
    ...
    def train(self, episodes=10000):
        ...
        for episode in range(episodes):
            ...
            if episode > 0 and episode % config.EXPLORATION_RESTART_EVERY == 0:
                if episode - last_restart_episode >= 200:  # 确保至少间隔200回合
            ...
            for step in range(config.MAX_STEPS):
                losses = self.system.train_step()
                if losses:
                    episode_losses['critic_loss'] += losses.get('critic_loss', 0)
                    episode_losses['actor_loss'] += losses.get('actor_loss', 0)
                    n_updates += 1

                # 使用探索模式（evaluate=False）引入随机性
                assignment, _, _ = self.system.assign(evaluate=False)
            ...
            # === 更新收敛监控 ===
            temp_assignment, _, _ = self.system.assign(evaluate=True)
            n_assigned = len(temp_assignment)
            status = monitor.update(episode, avg_episode_reward, n_assigned, temp_assignment, self.system.env)
            ...
            # 保存最佳模型（仅当全部分配时）
            if status.get('new_best') and config.SAVE_BEST and status.get('full_coverage'):
                best_model_state = {
                    'actor': self.system.masac.actor.state_dict(),
                    'critic': self.system.masac.critic.state_dict(),
                    'encoder': self.system.encoder.state_dict(),
                    'episode': episode,
                    'reward': status['window_mean'],
                    'assignment': temp_assignment,
                    'n_platforms': len(temp_assignment)
                }
                torch.save(best_model_state, 'best_model_checkpoint.pth')
                print(f"\n💾 新最佳模型保存: 回合{episode}, 奖励{status['window_mean']:.3f}, "
                      f"分配覆盖{status['n_platforms_assigned']}/{config.N_PLATFORMS}")
                for param_group in self.system.masac.actor_opt.param_groups:
                    param_group['lr'] = config.LR_ACTOR  # 恢复基础学习率

            # 记录历史数据
            self.history['rewards'].append(avg_episode_reward)
            self.history['platform_coverage'].append(len(temp_assignment))
            self.history['target_coverage'].append(n_assigned)
            ...
            if (episode + 1) % 20 == 0:  # 每20回合打印一次，更密集监控
                ...
                avg_unique_targets = np.mean([
                    len(set(a.values())) for a in recent_assignments
                ])
                print(f"\n{'=' * 60}")
                ...
                if (episode + 1) % 500 == 0:
                    ...
                    for eval_i in range(10):
                        ...
                        test_reward = self.system.calc_reward(test_assign)
                        eval_rewards.append(test_reward)
                        eval_unique_targets.append(len(set(test_assign.values())) if test_assign else 0)
                        eval_platform_coverage.append(len(test_assign))
            ...
            # 早停判断（仅当手动触发过且模型稳定时）
            if best_model_state is not None and status.get('should_stop', False) and trigger_status[
                'trigger_count'] > 0:
                try:
                    current_actor_dim = self.system.masac.actor.net[-1].weight.shape[0]
                    saved_actor_dim = best_model_state['actor']['net.2.weight'].shape[0]

                    if current_actor_dim != saved_actor_dim:
                        print(f"⚠️  检测到维度变化（当前{current_actor_dim} vs 保存{saved_actor_dim}），执行权重迁移...")
                        with torch.no_grad():
                            current_layer = self.system.masac.actor.net[-1]
                            saved_weight = best_model_state['actor']['net.2.weight'].to(self.system.device)
                            saved_bias = best_model_state['actor']['net.2.bias'].to(self.system.device)

                            min_dim = min(current_actor_dim, saved_actor_dim)
                            current_layer.weight[:min_dim].copy_(saved_weight[:min_dim])
                            current_layer.bias[:min_dim].copy_(saved_bias[:min_dim])

                            if current_actor_dim > saved_actor_dim:
                                current_layer.weight[saved_actor_dim:].fill_(0.01)
                                current_layer.bias[saved_actor_dim:].fill_(-0.1)

                        print(f"✅ 权重迁移完成，重置收敛计数器继续训练...")
                        monitor.patience_counter = 0
                        monitor.converged = False
                        status['should_stop'] = False
                    else:
                        self.system.masac.actor.load_state_dict(best_model_state['actor'])
                        self.system.masac.critic.load_state_dict(best_model_state['critic'])
                        self.system.encoder.load_state_dict(best_model_state['encoder'])
                        print("✅ 已恢复最佳模型状态，训练完成")
                        break
        ...
        if self.recent_best_episode > 0:
            ...
            print(
                f"   目标分配: {len(set(self.recent_best_assignment.values())) if self.recent_best_assignment else 0}/{config.N_TARGETS}")
    ...
    def _final_evaluation(self):
        """最终评估（提取为辅助方法保持代码整洁）"""
        print(f"\n{'=' * 60}")
        print("执行最终评估：强制要求全平台分配...")

        best_final_assignment = {}
        best_coverage = 0
        max_attempts = 20

        for attempt in range(max_attempts):
            final_assignment, _, _ = self.system.assign(evaluate=True)
            coverage = len(final_assignment)

            if coverage == len(self.env.platforms):
                print(f"✅ 第{attempt + 1}次尝试：成功获得全部分配方案")
                best_final_assignment = final_assignment
                break
            elif coverage > best_coverage:
                best_coverage = coverage
                best_final_assignment = final_assignment

            if (attempt + 1) % 5 == 0 and coverage < len(self.env.platforms):
                print(f"⚠️ 第{attempt + 1}次尝试：部分分配({coverage}/{len(self.env.platforms)})，调整策略...")
                old_epsilon = self.system.masac.epsilon
                self.system.masac.epsilon = 0.05
                self.env.reset()
                self.system.masac.epsilon = old_epsilon

        if len(best_final_assignment) < len(self.env.platforms):
            print(f"⚠️ 警告：最终评估未能达成全部分配，返回最优部分分配({best_coverage}/{len(self.env.platforms)})")
        else:
            print(f"✅ 最终评估完成：全部分配验证通过")
        ...
        print(f"\n📅 生成最终甘特图（最近{display_episodes}回合）...")
    ...
    def visualize_3d(self, assignment: Dict[int, List[int]]):
        ...
        # 图例
        legend_elements = [
            Line2D([0], [0], marker='o', color='w', markerfacecolor=Platform.COLORS['UAV'],
                   markersize=12, label='UAV', markeredgecolor='black'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor=Platform.COLORS['UCAV'],
                   markersize=12, label='UCAV', markeredgecolor='black'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor=Platform.COLORS['USV'],
                   markersize=12, label='USV', markeredgecolor='black'),
            Line2D([0], [0], marker='^', color='w', markerfacecolor=Target.COLORS['attack'],
                   markersize=12, label='Attack Target', markeredgecolor='black'),
            Line2D([0], [0], marker='s', color='w', markerfacecolor=Target.COLORS['reconnaissance'],
                   markersize=12, label='Recon Target', markeredgecolor='black'),
            Line2D([0], [0], marker='d', color='w', markerfacecolor=Target.COLORS['electronic'],
                   markersize=12, label='Electronic Target', markeredgecolor='black'),
            Line2D([0], [0], color='red', lw=3, alpha=0.3, label='Threat Zone'),
            Line2D([0], [0], color='black', lw=2, label='Assignment'),
        ]
    ...
    def _plot_temporal_dependencies_simple(self, assignment: Dict[int, List[int]], episode: int):
        """
        ...
        # 去重（一个目标可能被多个平台分配？根据实际情况决定是否需要）
        assigned_targets = list(set(assigned_targets))

        # [修复位置B] 构建反向映射（平台->目标列表）
        platform_to_targets = {pid: (tids if isinstance(tids, (list, tuple)) else [tids])
                               for pid, tids in assignment.items()}

        # 按任务类型分组布局
        task_types = ['electronic', 'reconnaissance', 'attack']
        ...
        for tid in assigned_targets:
            if tid < len(self.env.targets):  # 安全检查
        ...
        print(f"🔗 时序依赖图已生成: {filename}")
        print(f"   ├─ 分配任务数: {len(assigned_targets)}")
        print(f"   ├─ 时序约束数: {total_constraints}")
        print(f"   └─ 约束满足率: {satisfaction_rate:.1%}")
    ...
    def _plot_assignment_gantt(self, episode: int, recent_episodes: int = 500):
        ...
        print(f"\n📅 时序甘特图已生成: {filename}")
        print(f"   ├─ 展示范围: 回合 {start_idx}-{episode} (共{len(recent_assignments)}回合)")
        print(f"   ├─ 平均覆盖率: {recent_coverage:.1%}")
        print(f"   └─ 平均时序违反: {recent_violations:.2f}次/回合")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Key Fixes:")
    print(" ✓ Added missing Environment base class")
    print(" ✓ Fixed Trainer/TerrainTrainer nested class definition")
    print(" ✓ Removed incorrect super() calls")
    print(" ✓ Fixed type annotations")
    print(" ✓ Added file not found handling for terrain data")
    print(" ✓ Added bounds checking for array accesses")
    print(" ✓ Merged duplicate visualize_3d methods")
    print("=" * 70)

    trainer = TerrainTrainer(use_real_terrain=False)  # 明确使用模拟
    final_assignment = trainer.train(episodes=10000)
